...
export default function Inventory() {
  ...
  const [newStorage, setNewStorage] = useState({ name: "", amount: "" });
  ...
  // Effect to fetch storage data on component mount
  useEffect(() => {
    fetchStorageData();
  }, []);
  ...
  async function addItem() {
    try {
      const upperCaseName = newStorage.name.toUpperCase(); // Convert name to uppercase
      const newItem = { ...newStorage, name: upperCaseName };
      ...
      setNewStorage({ name: "", amount: "" });
      setShowAddForm(false);

      // Update storage data after adding new item
      fetchStorageData();
    } catch (error) {
  ...
  // Function to update the amount of an existing item
  async function updateItem() {
    try {
      ...
      // Update storage data after updating item
      fetchStorageData();
    } catch (error) {
  ...
  async function updateRecentItem(id) {
    try {
      ...
      // Update storage data after updating recent item
      fetchStorageData();
    } catch (error) {
      console.error("Error updating item:", error);
      alert("Error updating item!");
  ...
  async function deleteItem() {
    try {
      ...
      // Update storage data after deleting item
      fetchStorageData();
    } catch (error) {
  ...
  const handleSearch = async (event) => {
    ...
    if (query === "") {
      ...
      const filteredItems = storageData.filter(
      ...
      {showAddForm && (
        <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-50">
          <div className="w-full max-w-lg p-4 bg-gray-700 rounded-lg shadow-lg">
            <h2 className="text-xl font-bold mb-4 text-white">Add New Storage</h2>
            <form
              onSubmit={(e) => {
                e.preventDefault();
                addItem();
            ...
            >
              ...
              <div className="mb-4">
                ...
                <select
                  id="newAmount"
                  value={newStorage.amount}
                  onChange={(e) =>
                    setNewStorage({ ...newStorage, amount: e.target.value })
                  }
                  className="w-full text-black px-3 py-2 placeholder-gray-400 border rounded-lg focus:outline-none focus:ring focus:border-blue-400"
                >
                  <option value="" disabled>Select amount</option>
